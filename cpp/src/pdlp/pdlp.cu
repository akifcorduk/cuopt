    ...
    inside_mip_{false}
{
  ...
  for (auto& quality : best_primal_quality_so_far_) {
    quality.primal_objective = (op_problem_scaled_.maximize) ? -std::numeric_limits<f_t>::infinity()
  ...
  bool use_kkt)
{
  ...
  const auto& best_candidate = average_quality
                                 ? get_best_quality(current_quality, *average_quality, use_kkt)
                                 : current_quality;
...
// When called in the context of regular termination, the status is already stored in the current
// termination strategy When called in the context of limit reached (time or iteration), the status
// is set to the fallback status
template <typename i_t, typename f_t>
void pdlp_solver_t<i_t, f_t>::snapshot_climber_into_return(
  size_t i, std::optional<pdlp_termination_status_t> status_to_set)
{
  ...
  // We use is_done to make since this function may be called in the context of time or iteration
  // limit
  if (sb_view_.is_valid() && current_termination_strategy_.is_done(term, accept_pf)) {
    sb_view_.mark_solved(local_idx);
  }
}
...
void pdlp_solver_t<i_t, f_t>::record_best_primal_so_far_batch()
{
  ...
  // the kernel write every active climber (not only the just-terminated ones) at its
  // original_index. We don't do conversion to host per climber in a for loop as it would be too
  // slow.
  ...
  for (size_t i = 0; i < climber_strategies_.size(); ++i) {
    ...
    // l2_primal_residual / l2_dual_residual fields when settings_.per_constraint_residual is on,
    // and the L2 residual otherwise, so these comparison quantities already match the active
    // criterion.
