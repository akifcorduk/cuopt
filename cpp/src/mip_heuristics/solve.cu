                                   ...
                                   mip_solver_settings_t<i_t, f_t> const& settings_const)
{
  ...
  // Heuristic-only runs the GPU diversity workflow without B&B; a single thread is enough
  // (root LP is PDLP-only in diversity_manager::run_solver).
  if (num_threads < 2 && !settings_const.heuristics_only) {
