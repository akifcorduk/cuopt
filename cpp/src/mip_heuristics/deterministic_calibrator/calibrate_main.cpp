...
using namespace cuopt::mathematical_optimization::mip::calib;
...
// Build fitter options, allowing CALIB_HUBER / CALIB_L1 / CALIB_L2 / CALIB_PIN_MEAN env overrides
// so the robust/elastic-net knobs can be swept without recompiling.
...
void fit_two_kernel_device(const std::string& algo, const std::vector<std::string>& csvs)
{
  ...
  auto combined_stats = [&](
                          const std::vector<double>& ca, const std::vector<double>& cu, double k) {
    std::vector<double> r;
    for (std::size_t i = 0; i < act.size(); ++i) {
      std::vector<double> rawa(act[i].feat.begin() + 1, act[i].feat.end());
      std::vector<double> rawu(upd[i].feat.begin() + 1, upd[i].feat.end());
