...
constexpr const char* pre_bnb_arm_name = "PRE-BNB-SELECTIVE-FIRST-FINAL";
  ...
  const mip_solver_settings_t<i_t, f_t>& settings)
{
  ...
  CUOPT_LOG_INFO(
    "PRE_BNB_CONFIG variant=selective_first_final task_slots=%d bnb_threads=%d "
    "small_nnz_limit=%d row_limit=%d max_nnz_limit=%d time_limit=%.3fs node_limit=%d "
    "sb_simplex_iteration_limit=%d initial_root_sb=skipped initial_pseudocost=neutral "
    "diving=retained preserve_advanced_basis_dimensions=%d publication=first_plus_final_best",
    pre_presolve_primal_task_slots,
    pre_bnb_num_threads,
    pre_presolve_primal_small_nnz_limit,
    pre_presolve_primal_row_limit,
                         ...
                         bool stop_requested)
{
  CUOPT_LOG_INFO(
    ...
    "candidates_published=%zu stop_requested=%d",
    status,
    elapsed,
    nodes,
    generated,
...
template <typename i_t, typename f_t>
void run_pre_bnb_first_final(simplex::user_problem_t<i_t, f_t>& user_problem,
                             simplex::simplex_solver_settings_t<i_t, f_t>& bnb_settings,
                             pre_presolve_first_final_candidate_store_t<f_t>& candidate_store,
                             ...
                             std::atomic<bool>& stop_requested)
{
  ...
  try {
    ...
    if (retained_candidate.has_candidate && retained_candidate.submitted > 0 &&
        retained_candidate.solver_objective < retained_candidate.first_solver_objective &&
        incumbent_callback) {
      ...
      incumbent_callback(retained_candidate.solver_objective,
                         ...
                         pre_bnb_arm_name);
      candidate_store.mark_submitted();
    }
    const auto final_candidate = candidate_store.snapshot();

    const auto status_name = mip_status_to_string(status);
    log_pre_bnb_summary(status_name.c_str(),
                        toc(solve_start),
                        solution.nodes_explored,
                        final_candidate.generated,
                        final_candidate.submitted,
  ...
  early_incumbent_callback_t<f_t> incumbent_callback)
{
  ...
  const int num_rows = op_problem.get_n_constraints();
  const int nnz      = op_problem.get_nnz();
  if (!pre_presolve_primal_selected(num_rows, nnz)) {
    CUOPT_LOG_INFO(
      "PRE_BNB_SUMMARY eligible=0 reason=structural_gate rows=%d nnz=%d small_nnz_limit=%d "
      "row_limit=%d max_nnz_limit=%d",
      num_rows,
      nnz,
      pre_presolve_primal_small_nnz_limit,
      pre_presolve_primal_row_limit,
  ...
  auto candidate_store    = std::make_shared<pre_presolve_first_final_candidate_store_t<f_t>>();
  auto bnb_settings       = make_pre_bnb_settings<i_t, f_t>(settings);
  auto immediate_callback = incumbent_callback;
  bnb_settings.solution_callback = [candidate_store,
                                    immediate_callback = std::move(immediate_callback),
                                    objective_scale,
                                    objective_constant](std::vector<f_t>& assignment,
                                                        f_t solver_objective) mutable {
    if (!candidate_store->consider(solver_objective, assignment) || !immediate_callback) { return; }
    const f_t user_objective = objective_scale * (solver_objective + objective_constant);
    immediate_callback(solver_objective, user_objective, assignment, pre_bnb_arm_name);
    candidate_store->mark_submitted();
  };
  ...
  auto run_work = [user_problem,
    ...
    run_pre_bnb_first_final(*user_problem,
                            bnb_settings,
                            *candidate_store,
                            incumbent_callback,
                            objective_scale,
