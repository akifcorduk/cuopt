...
struct pre_bnb_shadow_candidate_t {
  ...
  early_incumbent_callback_t<f_t>)
{
  ...
  if (op_problem.get_nnz() > pre_bnb_nnz_limit) {
    CUOPT_LOG_INFO("PRE_BNB_SUMMARY eligible=0 reason=nnz_gate nnz=%d limit=%d",
                   op_problem.get_nnz(),
                   pre_bnb_nnz_limit);
    return;
  }
  ...
  CUOPT_LOG_INFO(
    "PRE_BNB_CONFIG variant=basis_preserve_shadow task_slots=%d bnb_threads=%d nnz_limit=%d "
  ...
  auto shadow_candidates         = std::make_shared<std::vector<pre_bnb_shadow_candidate_t<f_t>>>();
  auto shadow_candidates_mutex   = std::make_shared<std::mutex>();
  const f_t objective_scale      = user_problem->obj_scale;
  const f_t objective_constant   = user_problem->obj_constant;
  bnb_settings.solution_callback = [objective_scale,
                                    objective_constant,
                                    candidates_generated,
                                    shadow_candidates,
                                    shadow_candidates_mutex](std::vector<f_t>& assignment,
                                                             f_t solver_obj) {
    const f_t user_obj = objective_scale * (solver_obj + objective_constant);
    {
      std::lock_guard<std::mutex> lock(*shadow_candidates_mutex);
      shadow_candidates->push_back({solver_obj, user_obj, assignment});
    }
    candidates_generated->fetch_add(1, std::memory_order_relaxed);
  };

  work_ =
    [user_problem, bnb_settings, candidates_generated, shadow_candidates, shadow_candidates_mutex](
      ...
      CUOPT_LOG_INFO(
        "PRE_BNB_SUMMARY eligible=1 status=%s elapsed=%.3fs nodes=%d "
        "candidates_generated=%d candidates_published=0 stop_requested=%d",
      ...
      // Retain the same assignment-copy cost as an active callback, then discard every candidate
      // without exposing it to the shared incumbent pool.
      std::lock_guard<std::mutex> lock(*shadow_candidates_mutex);
