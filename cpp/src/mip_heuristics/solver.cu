...
solution_t<i_t, f_t> mip_solver_t<i_t, f_t>::run_solver()
{
  ...
  if (!context.settings.heuristics_only) {
    ...
    // Propagate the benchmark_info pointer so branch_and_bound can publish
    // root_lp_no_cuts / root_lp_with_cuts / cut_generation_time_sec for
    // MIPLIBGapStat. nullptr in production builds.
