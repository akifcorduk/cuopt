...
End
  ...
  per_climber_lb[0] = 10.0;
  per_climber_ub[0] = 10.0;
  per_climber_lb[1] = 20.0;
  per_climber_ub[1] = 20.0;
  per_climber_lb[2] = 0.0;
  per_climber_ub[2] = 200.0;
  // Climber 1: x == 10, y == 20, x + y == 90 (infeasible)
  per_climber_lb[3] = 10.0;
  per_climber_ub[3] = 10.0;
  per_climber_lb[4] = 20.0;
  per_climber_ub[4] = 20.0;
  per_climber_lb[5] = 90.0;
  per_climber_ub[5] = 90.0;
  return mps_data;
}
...
End
  ...
  per_climber_lb[0] = 100000.0;
  per_climber_ub[0] = 100000.0;
  per_climber_lb[1] = 0.0;
  per_climber_ub[1] = 0.0;
  // Climber 1: x + y == 1e5, x - y == 2e5 (infeasible -> iteration limit -> best-so-far path).
  per_climber_lb[2] = 100000.0;
  per_climber_ub[2] = 100000.0;
  per_climber_lb[3] = 200000.0;
  per_climber_ub[3] = 200000.0;
  return mps_data;
}
  ...
  double rel_tolerance)
{
  ...
  for (size_t i = 0; i < residual.size(); ++i) {
    ...
    const double viol = val < lb ? (lb - val) : (val > ub ? (val - ub) : 0.0);
...
TEST(pdlp_class, best_kkt_so_far_vs_best_primal_non_batch_stable3)
{
  ...
  // primal feasible, so both rankings operate in the "not feasible" regime for the whole
  // trajectory, where the min-primal-residual iterate and the min-KKT iterate genuinely diverge.
...
// The L2 primal criterion allows ||residual||_2 <= abs + rel * ||rhs||_2 ~= rel * 1e5, which is
// huge relative to c_small's per-row tolerance, so an L2-"optimal" iterate can leave c_small
// violated by far more than its per-row tolerance. The large objective coefficient keeps the
// dual/gap thresholds loose too, so termination is gated by the (loose) primal criterion rather
// than tight convergence. With per_constraint_residual=false the returned solution must FAIL the
// per-row sanity; with per_constraint_residual=true it must PASS it.
...
// vector of best qualities and the per-constraint criterion must drive both the *termination* of
// the optimal climber and the *selection* recorded for the limit-reaching climber. For each ranking
// (best primal / best kkt) and each residual mode (L2 / per-constraint) we check that the
// ill-scaled optimal climber (0) obeys the per-row criterion only under per_constraint_residual,
// while the infeasible climber (1) is still returned through the best-so-far path without clashing.
TEST(pdlp_class, per_constraint_residual_batch_best_so_far_enforces_per_row_stable3)
{
  ...
  auto check_optimal_climber0 = [&](const optimization_problem_solution_t<int, double>& sol,
                                    bool per_constraint) {
...
End
  ...
  // Force the batch to end on the time limit: leave the iteration limit unbounded and cap wall
  // time.
...
// B4 - batch, first primal feasible: climber 0 reaches primal feasibility and stops the whole
// batch; climber 1 (never feasible) must still return its snapshotted current iterate consistently
// with its reported objective (exercises the all-climber snapshot in check_batch_termination).
...
// criterion is the per-row relative L-inf residual instead of the L2 residual). Climbers differ
// only on constraint lower/upper bounds; first primal feasible must still return every climber's
// current iterate consistently with its reported objective.
TEST(pdlp_class,
     batch_first_primal_feasible_per_constraint_residual_returns_current_solution_stable3)
{
  const raft::handle_t handle_{};
...
TEST(pdlp_class, batched_fp_unified_projection_and_selection)
{
  ...
  for (int c = 0; c < batch_size; ++c) {
    ...
    const auto primal =
      host_copy(extract_subvector(
                  sol.get_primal_solution(), static_cast<size_t>(c) * n_vars_total, n_vars_total),
                stream);
    handle_.sync_stream();
